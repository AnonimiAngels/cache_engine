// File: inc/cache_engine/policies/eviction/mru_policy.hpp
...
namespace cache_engine
{
	namespace policies
	{
		namespace eviction
		{
			template <typename key_t, typename value_t> class mru_policy
			{
			  public:
				using self_t		= mru_policy<key_t, value_t>;
			  ...
			  public:
				...
				mru_policy(const self_t&)				 = delete;
				auto operator=(const self_t&) -> self_t& = delete;

				mru_policy(self_t&& p_other) noexcept : m_access_list(std::move(p_other.m_access_list)), m_key_to_iterator(std::move(p_other.m_key_to_iterator)) {}
			  ...
			  public:
				...
				auto select_victim() -> key_t
				{
					...
					// MRU evicts the most recently used (front of list)
