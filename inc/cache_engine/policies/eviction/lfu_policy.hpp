// File: inc/cache_engine/policies/eviction/lfu_policy.hpp
...
namespace cache_engine
{
	namespace policies
	{
		namespace eviction
		{
			template <typename key_t, typename value_t> class lfu_policy
			{
			  public:
				using self_t	  = lfu_policy<key_t, value_t>;
			  ...
			  public:
				...
				lfu_policy(const self_t&)				 = delete;
				auto operator=(const self_t&) -> self_t& = delete;

				lfu_policy(self_t&& p_other) noexcept : m_key_to_frequency(std::move(p_other.m_key_to_frequency)), m_frequency_to_keys(std::move(p_other.m_frequency_to_keys)) {}
			  ...
			  public:
				...
				auto select_victim() -> key_t
				{
					...
					// Select least frequently used (lowest frequency)
					auto& least_freq_list = m_frequency_to_keys.begin()->second;
